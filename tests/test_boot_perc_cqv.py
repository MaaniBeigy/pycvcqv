"""Tests for the bootstrap percentile CI for the cqv (`method='perc'`)."""
...
# R `cvcqv` README values for `percent` 95% CI: 45.625, 17.122, 68.683
REFERENCE_DATA = [
    0.2,
    0.5,
    1.1,
...
def test_boot_perc_cqv_in_ballpark_of_r_reference():
    """With R=10000, the percentile-bootstrap CI sits within a few percent of R."""
    result = cqv(
        data=REFERENCE_DATA,
        method="perc",
        multiplier=100,
    ...
    assert result["lower"] == pytest.approx(17.122, abs=3.0)
    assert result["upper"] == pytest.approx(68.683, abs=3.0)


def test_boot_perc_cqv_is_reproducible_with_random_state():
    """Same seed ⇒ identical CI bounds."""
    a = _boot_perc_cqv_confidence_interval(
        data=REFERENCE_DATA, multiplier=100, num_replicates=500, random_state=7
    )
    b = _boot_perc_cqv_confidence_interval(
        data=REFERENCE_DATA, multiplier=100, num_replicates=500, random_state=7
    )
    assert a == b


def test_boot_perc_cqv_accepts_external_generator():
...
def test_boot_perc_cqv_bounds_bracket_the_point_estimate():
    """The CI brackets the point estimate."""
    result = _boot_perc_cqv_confidence_interval(
        data=REFERENCE_DATA, multiplier=100, num_replicates=500, random_state=2
    )
    assert result["lower"] < result["cqv"] < result["upper"]


def test_boot_perc_cqv_skipna_drops_nans():
    """skipna=True silently drops NaNs."""
    data_with_nan = [*REFERENCE_DATA, float("nan")]
    with_nans = _boot_perc_cqv_confidence_interval(
        data=data_with_nan, multiplier=100, num_replicates=200, random_state=5
    )
    without_nans = _boot_perc_cqv_confidence_interval(
        data=REFERENCE_DATA, multiplier=100, num_replicates=200, random_state=5
    )
    assert with_nans == without_nans


def test_boot_perc_cqv_skipna_false_raises_on_nan():
    """skipna=False raises on NaN."""
    data_with_nan = [*REFERENCE_DATA, float("nan")]
    with pytest.raises(ValueError, match="missing values"):
        _boot_perc_cqv_confidence_interval(
            data=data_with_nan, multiplier=100, skipna=False
        )


def test_boot_perc_cqv_too_short_raises():
    """Bootstrap perc CQV CI requires at least 2 observations."""
    with pytest.raises(ValueError, match="at least 2 observations"):
        _boot_perc_cqv_confidence_interval(data=[1.0], multiplier=100)


def test_boot_perc_cqv_too_few_replicates_raises():
