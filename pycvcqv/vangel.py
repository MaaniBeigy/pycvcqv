"""Vangel confidence interval for the coefficient of variation."""
...
# -------------------------------- function definition --------------------------------
def _vangel_cv_confidence_interval(
    data: (
        pd.Series
        | ArrayInt
        | ArrayFloat
    ...
    """Compute Vangel's confidence interval for the coefficient of variation (CV).

    Vangel's interval is a modification of McKay's interval that improves
    coverage for moderate CV values. With v = n - 1 and chi-square quantiles
    u_high = qchisq(1 - alpha/2, v), u_low = qchisq(alpha/2, v):

        lower = cv / sqrt(((u_high + 1)/v - 1) * cv^2 + u_high / v)
        upper = cv / sqrt(((u_low  + 1)/v - 1) * cv^2 + u_low  / v)
    ...
    _length = len(_data)
    if _length < 2:
        raise ValueError("Vangel CI requires at least 2 observations.")
    ...
    lower_denom = math.sqrt(
        ((u_high + 1) / degrees_of_freedom - 1) * cv_squared
        + u_high / degrees_of_freedom
    )
    upper_denom = math.sqrt(
        ((u_low + 1) / degrees_of_freedom - 1) * cv_squared + u_low / degrees_of_freedom
    )
