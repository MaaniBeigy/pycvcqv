...
poetry run python3 -m radon mi pycvcqv -s >>.logs/maintainability.txt
export maintainability=$(poetry run python3 -m radon mi pycvcqv -s | grep -Po '(?<=\().*(?=\))' | perl -lane '$a+=$_ for(@F);$f+=scalar(@F);END{print $a/$f}' | xargs printf "%.1f%%\n")
